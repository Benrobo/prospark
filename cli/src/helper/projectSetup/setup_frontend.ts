...
class SetupFrontend extends ProjectBaseSetup {
  ...
  protected async configureNextjsTailwindCss(path: string) {
    if (!fs.pathExistsSync(path)) return;
  ...
  protected async isReactAndTailwind(promptInput: ProjectOptions) {
    ...
    try {
      ...
      const pkgJsonData: any = await this.configureReactPkgJson(
        newPkgJsonPath,
        projectName,
  ...
  protected async isSvelteAndTailwindCss(promptInput: ProjectOptions) {
    ...
    try {
      ...
      const pkgJsonData: any = await this.configureSveltePkgJson(
        newPkgJsonPath,
        projectName,
        frontendStyling as string
      );
