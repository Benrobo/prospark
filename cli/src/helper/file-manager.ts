...
export function removeFile(dest_path: string, fileName: string) {
  try {
    if (!fs.existsSync(dest_path)) {
      logger.error(`failed to delete file, destination path doesn't exists.`);
      return;
    }
    const file = `${dest_path}/${fileName}`;
    fs.removeSync(file);
...
export async function updateFileContent(
  ...
  try {
    ...
    if (!fs.existsSync(path_to_file)) {
      logger.error(
        `something went wrong, try again: failed to update file, path_to_file (${path_to_file}) path doesn't exists.`
      );
      return;
    }
